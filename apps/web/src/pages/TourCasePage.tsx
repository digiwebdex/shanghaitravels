...
export default function TourCasePage() {
  ...
  if (!app) {
    return (
      <div className="p-5">
        ...
        <Link to="/tours" className="text-[11px] text-[var(--accent)] font-semibold">
          ← Tours
        </Link>
      </div>
    );
  ...
  if (app.serviceType !== "tour") {
    return (
      <div className="p-5">
        <ErrorBanner message={`This case is serviceType=${app.serviceType}, not tour.`} />
        <Link to="/tours" className="text-[11px] text-[var(--accent)] font-semibold">
          ← Tours
        </Link>
      </div>
...
<section className="bg-white rounded-xl border border-[var(--border)] p-4">
          ...
          <div className="mt-4 flex flex-wrap gap-2 items-end">
            <div className="flex-1 min-w-[200px]">
              <input
                ...
                placeholder="Note / supplier costing ref…"
        ...
        <CaseFinanceCard
          ...
          setError={setError}
          setOk={setOk}
          defaultDescription={`${app.title || "Tour"} — package fee`}
        />
