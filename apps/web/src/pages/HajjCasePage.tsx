...
import { HajjDetailCard } from "@/components/cases/HajjDetailCard";
import { HajjOpsCard } from "@/components/cases/HajjOpsCard";
...
export default function HajjCasePage() {
  ...
  if (!app) {
    return (
      <div className="p-5">
        ...
        <Link to="/hajj" className="text-[11px] text-[var(--accent)] font-semibold">
          ← Hajj & Umrah
        </Link>
      </div>
    );
  ...
  if (app.serviceType !== "hajj" && app.serviceType !== "umrah") {
    return (
      <div className="p-5">
        <ErrorBanner message={`This case is serviceType=${app.serviceType}, not hajj/umrah.`} />
        <Link to="/hajj" className="text-[11px] text-[var(--accent)] font-semibold">
          ← Hajj & Umrah
        </Link>
      </div>
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        subtitle={`${app.customer?.fullName || "Customer"} · ${app.title || "Hajj/Umrah"} · stage ${app.currentStage}/${app.totalStages}`}
        breadcrumb={[{ label: "Hajj & Umrah", to: "/hajj" }, { label: app.referenceNo }]}
        actions={
...
<section className="bg-white rounded-xl border border-[var(--border)] p-4">
          ...
          <div className="mt-4 flex flex-wrap gap-2 items-end">
            <div className="flex-1 min-w-[200px]">
              <input
                ...
                placeholder="Note / pilgrim / supplier ref…"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <HajjDetailCard
            ...
            serviceType={app.serviceType === "umrah" ? "umrah" : "hajj"}
            detail={app.hajjUmrah}
            onSaved={reload}
            setError={setError}
            setOk={setOk}
          />
        ...
        <CaseFinanceCard
          ...
          setError={setError}
          setOk={setOk}
          defaultDescription={`${app.title || "Hajj/Umrah"} — Hajj/Umrah package fee`}
        />
