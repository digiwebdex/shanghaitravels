      ...
      <div className="p-5 max-w-[1200px]">
        <div className="flex items-end justify-between gap-3 mb-3 flex-wrap">
          <div>
            <h1 className="text-[16px] font-bold text-slate-800 flex items-center gap-2">
              <Car size={16} className="text-amber-600" /> Transport
            </h1>
            <p className="text-[11px] text-slate-500 mt-0.5">
              {total} bookings · supplier-purchased transfers (no fleet)
            </p>
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          ...
          {loading ? (
            ...
            <EmptyState title="No transport bookings" hint="Create a booking or adjust filters." />
          ) : (
...
export function NewTransportCasePage() {
  ...
  return (
    <div className="p-5 max-w-[640px]">
      <TransportModuleNav />
      <h1 className="text-[16px] font-bold text-slate-800 mb-1">New transport booking</h1>
      <p className="text-[11px] text-slate-500 mb-4">
        Creates an Application and applies the active transport workflow template.
      </p>
      <ErrorBanner message={error} />
      <form
