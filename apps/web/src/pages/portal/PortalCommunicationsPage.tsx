...
import { validateSupportRequest } from "@/lib/portal";
...
export default function PortalCommunicationsPage() {
  ...
  async function load() {
    const r = await customerPortalApi.communications();
  ...
  async function submit(e: FormEvent) {
    e.preventDefault();
    const bad = validateSupportRequest({ subject, body });
    if (bad) {
      setError(bad);
      return;
    }
    try {
      await customerPortalApi.support({ subject, body });
      setOk("Support request submitted");
      setSubject("");
      setBody("");
      await load();
    } catch (err) {
  ...
  return (
    <div className="p-5 max-w-5xl space-y-4">
      ...
      <form onSubmit={submit} className="bg-white border rounded-xl p-4 space-y-2">
        <h2 className="text-[12px] font-bold">Support request</h2>
