...
export default function AgentPackagesBrowsePage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await agentPortalApi.listPackages();
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {rows.map((p) => (
            <PackageCard
              ...
              variant="portal"
              showCommission
              bookPath={`/portal/agent/packages/${p.slug}/book`}
              detailPath={`/portal/agent/packages/${p.slug}`}
            />
          ))}
        </div>
      )}
