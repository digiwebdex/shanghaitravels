...
import { validateAgentSupport } from "@/lib/agentPortal";
...
export default function AgentCommunicationsPage() {
  ...
  async function load() {
    const r = await agentPortalApi.communications();
  ...
  async function submit(e: FormEvent) {
    e.preventDefault();
    const bad = validateAgentSupport({ subject, body });
    if (bad) {
      setError(bad);
      return;
    }
    try {
      await agentPortalApi.support({ subject, body });
