...
export default function CorporateForgotPage() {
  ...
  async function reset(e: FormEvent) {
    e.preventDefault();
    try {
      await corporatePortalApi.reset(email.trim(), code.trim(), password);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[var(--background)] p-4">
      <form onSubmit={step === "request" ? request : reset} className="w-full max-w-sm bg-white border rounded-xl p-5 space-y-3">
        <h1 className="text-[16px] font-bold">Corporate password reset</h1>
        ...
        <Link className="text-[11px] text-[var(--accent)] underline" to="/portal/corporate/login">
          Login
        </Link>
      </form>
    </div>
