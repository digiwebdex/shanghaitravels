...
export default function HotelsListPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(e instanceof ApiError ? e.message : "Failed to load hotel bookings");
    } finally {
      setLoading(false);
    }
  }, [q, status, agentId]);
  ...
  return (
    <ListPageShell
      ...
      icon={Building2}
      title="Hotels"
      subtitle="Manual property / supplier fulfilment (no bed-bank)."
      breadcrumb={[{ label: "Bookings", to: "/hotels" }, { label: "Hotels" }]}
...
export function NewHotelCasePage() {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      const app = await applicationsApi.create({
        ...
        customerId,
        priority,
        title: title.trim() || `Hotel — ${cust?.fullName || "customer"}`,
        direction: "outbound",
      });
      navigate(`/hotels/${app.id}`, { replace: true });
    } catch (err) {
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        subtitle="Creates an application and applies the active hotel workflow template."
        breadcrumb={[
          { label: "Bookings", to: "/hotels" },
      ...
      <Surface padded className="max-w-xl space-y-3">
        <form onSubmit={(e) => void submit(e)} className="space-y-3">
          <div>
            <label className={labelCls} htmlFor="ht-customer">
              Customer *
            </label>
            <select
              id="ht-customer"
          ...
          <div>
            ...
            <select id="ht-priority" className={inputCls} value={priority} onChange={(e) => setPriority(e.target.value)}>
          ...
          <div>
            <label className={labelCls} htmlFor="ht-title">
              Title
            </label>
            <input
              id="ht-title"
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button type="button" onClick={() => navigate("/hotels")} className={btnGhost}>
              Cancel
            </button>
          </div>
        </form>
