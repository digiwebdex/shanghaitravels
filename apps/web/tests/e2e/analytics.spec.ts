...
test.describe("CRM analytics", () => {
  test("login and open executive, sales, reports", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Executive analytics/i })).toBeVisible({ timeout: 20_000 });

    await page.goto("./#/analytics/sales");
    await expect(page.getByRole("heading", { name: /Sales analytics/i })).toBeVisible({ timeout: 20_000 });

    await page.goto("./#/analytics/reports");
    await expect(page.getByRole("heading", { name: /Analytics reports/i })).toBeVisible({ timeout: 20_000 });
  });
});
