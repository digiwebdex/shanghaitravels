...
test.describe("AR / AP module", () => {
  test("login and open AR, AP, reports", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Accounts Receivable/i })).toBeVisible({
      timeout: 20_000,
    });

    await page.goto("./#/finance/ap");
    await expect(page.getByRole("heading", { name: /Accounts Payable/i })).toBeVisible({
      timeout: 20_000,
    });

    await page.goto("./#/finance/ar-ap-reports");
    await expect(page.getByRole("heading", { name: /AR \/ AP reports/i })).toBeVisible({
      timeout: 20_000,
    });
  });
});
