...
const REFRESH_PATH = "/api/portal/agent";

function setCookies(res: Response, t: { access: string; refresh: string }) {
  res.cookie("st_agent", t.access, { httpOnly: true, secure: secure(), sameSite: "lax", path: "/", maxAge: accessMaxAge });
  res.cookie("st_agent_refresh", t.refresh, {
    httpOnly: true,
    secure: secure(),
    sameSite: "lax",
    path: REFRESH_PATH,
...
function clearCookies(res: Response) {
  res.clearCookie("st_agent", { path: "/" });
  res.clearCookie("st_agent_refresh", { path: REFRESH_PATH });
}
...
export class AgentAuthController {
  ...
  async refresh(@Req() req: Request, @Res({ passthrough: true }) res: Response) {
    const tokens = await this.auth.refresh(req.cookies?.st_agent_refresh, meta(req));
    setCookies(res, tokens);
    return { ok: true };
  }
  ...
  async logout(@Req() req: Request, @Res({ passthrough: true }) res: Response) {
    await this.auth.logout(req.cookies?.st_agent_refresh);
    clearCookies(res);
    return { ok: true };
  }
