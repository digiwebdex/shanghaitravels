...
import { LoginDto, EmailDto, ResetPasswordDto, EmailCodeDto } from "../common/dto/auth.dto";

const secure = () => process.env.COOKIE_SECURE !== "false";
const accessMaxAge = 15 * 60 * 1000;
const refreshMaxAge = Number(process.env.REFRESH_TTL_DAYS || 30) * 864e5;
const REFRESH_PATH = "/api/portal/corporate";

function setCookies(res: Response, t: { access: string; refresh: string }) {
  res.cookie("st_corporate", t.access, { httpOnly: true, secure: secure(), sameSite: "lax", path: "/", maxAge: accessMaxAge });
  res.cookie("st_corporate_refresh", t.refresh, {
    httpOnly: true,
    secure: secure(),
    sameSite: "lax",
    path: REFRESH_PATH,
...
function clearCookies(res: Response) {
  res.clearCookie("st_corporate", { path: "/" });
  res.clearCookie("st_corporate_refresh", { path: REFRESH_PATH });
}
...
export class CorporateAuthController {
  ...
  async refresh(@Req() req: Request, @Res({ passthrough: true }) res: Response) {
    const tokens = await this.auth.refresh(req.cookies?.st_corporate_refresh, meta(req));
    setCookies(res, tokens);
    return { ok: true };
  }
  ...
  async logout(@Req() req: Request, @Res({ passthrough: true }) res: Response) {
    await this.auth.logout(req.cookies?.st_corporate_refresh);
    clearCookies(res);
    return { ok: true };
  }
