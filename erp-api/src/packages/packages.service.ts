...
export class PackagesService {
  ...
  async agentBook(agentId: string, packageId: string, dto: any) {
    ...
    if (customerId) {
      // V6 Wave 1 (closes C4): an agent may only book for a customer they own.
      const c = await this.prisma.customer.findFirst({ where: { id: customerId, deletedAt: null } });
      const owned = c && (c.primaryAgentId === agentId || c.secondaryAgentId === agentId || c.createdBy === `agent:${agentId}`);
      if (!owned) throw new NotFoundException("Customer not found"); // NotFound, not Forbidden — don't confirm foreign IDs
    }
    if (!customerId) {
      ...
      const customer = await this.prisma.customer.create({
        data: {
          ...
          primaryAgentId: agentId, // V6 Wave 1: creating agent owns the customer
        },
      });
      await this.prisma.customerAssignment.create({ data: { customerId: customer.id, toAgentId: agentId, role: "primary", action: "assign", reason: "agent package booking", actorAgentId: agentId } });
